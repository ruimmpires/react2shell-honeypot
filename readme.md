...
##🏗️ Build & Deploy###1. Build the Image```bash
...
###3. Run the ContainerWe run on Port **8080** to avoid conflicts with existing services.
...
##🧪 VerificationTest the honeypot from a local terminal:
...
1. **Critical Alerts:** Count should increase by 5.
2. **Attack Volume:** New bars appearing in the chart.
3. **Payload Table:** You should see a mix of clean URLs (from Test 1 & 4) and raw commands (from Test 2, 3, & 5).
